...
The marketplace-admin app is gaining a **browser-like tab workspace** (parallel work contexts that persist across refresh and autosave drafts; catalogs now, operations/wizards later). Key rules for working in it:

- **TanStack Query WRAPS Entifix, never replaces it.** Server state (list rows, entities) is cached by `@tanstack/react-query` in `@r10c/entifix-react-integration`; the Effect UC/adapter pattern is untouched — `queryFn`/`mutationFn` just run `Effect.runPromise(Effect.provide(uc, ctx.pipe(Context.add(EntityLoadRequestTag, req))))` over the same Tags. `useDataLoading` keeps its `{ uc, ctx }` public shape (only its guts move onto `useQuery`), so organisms are unchanged. Query keys = `[envelopeEntityName(Ctor), serializeLoadRequestParams(req).toString()]`. Mutations are optimistic (`onMutate` patch + snapshot, `onError` rollback, `onSettled` invalidate). `QueryClientProvider` mounts in the app's `providers.tsx`. Don't add a second server-state cache and don't reach for `useState`+manual fetch.
- **Client state → Zustand + IndexedDB** (`zustand-indexeddb`, multi-store keyed by `persist` name): `tabsStore` (set/order/active/dirty) and `draftsStore` (keyed by **address**, debounced autosave, **workspace-host only**). `UiPreferencesStore` migrated localStorage→IndexedDB (same Effect port, new `makeIndexedDbUiPreferencesStore` + layer) so all persisted client state shares one store.
- **Dual-host pages**: a page is a pure `PageView({ addr })` (no `useParams`/`usePathname`/router reads) rendered by **either** a Next route host **or** a workspace tab. The **`TabKind` registry** (`match`/`toPath`/`render`/`title`/`singleton`, in `shells-next-common`) is the single URL⇄address⇄view mapper; route files are thin adapters. URL scheme is one fixed route `/workspace?tab=<kind>:<args>` (`?tab=` present ⇒ it's a tab; unknown kind ⇒ fallback, no crash). Nav is intent-based via `EntityNavHost` (tab host → `openOrFocus`; route host → `router.push`).
- **Reactive**: a framework-free `ReactiveChannel` port (`Context.Tag`, mockable now) feeds `queryClient.invalidateQueries`/`setQueryData` when the coming WebSocket lands. **Skeletons** are server-rendered chrome + active-tab (from `?tab=`); skeleton on first load only, then stale-while-revalidate (no spinners). `TopBar`/`Skeleton`/`Menu` are new agnostic controls; stateful wiring stays in the shells. Full design: [docs/WORKSPACE-TABS.md](docs/WORKSPACE-TABS.md). See [[workspace-tabs-design]].
