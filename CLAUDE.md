...
`@r10c/entifix-ts-business` defines the *contracts* and the **use-case factories** built on the [Effect](https://effect.website) library:
...
`@r10c/entifix-ts-rest-client` converts an `EntityConstructor` into an `EntityRepository` over HTTP via `buildEntityRestAdapter(Ctor)` — used by Next shells to back the `EntityRepositoryTag` on the client.

`@r10c/entifix-react-controls` / `-helpers` / `-integration` are the React side: generic UI primitives (`Table`), helpers, and Effect-aware hooks (`useDataLoading({ uc, ctx })`) that run a UC factory against an adapter context.
...
When adding a new domain entity, the layered path is: define the entity in `business/ts/<domain>` (decorate with `@entity`/`@accessor`), then build the UI organism in `implementation/<domain>/react`, wire the page in `shells/next/<shell>`, and register the adapter in the shell's adapters factory.
