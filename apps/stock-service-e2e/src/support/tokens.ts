 ...
 * precise defect this suite exists to prevent. `STOCK_ORGANIZATION_ID` is how a
 * live run names it; the local fleet seeds `demo-organization`.
 */
export const E2E_ORGANIZATION_ID =
  process.env['STOCK_ORGANIZATION_ID'] ??
  (isMockProfile() ? 'e2e-organization' : 'demo-organization');

/**
 * The crossing secret `POST /api/reservation` expects.
 *
...
export const E2E_CROSSING_TOKEN =
  process.env['STOCK_CROSSING_TOKEN'] ?? 'e2e-crossing-token';
