...
| **Refund**                | Money going back for an order. A **record of its own**, never a status on the `Payment` it reverses: the evidence that a customer was charged has to survive the money going back. One per capture — partial refunds are out of v1 scope.                                                                                                               | ours          |
...
| **CommissionEntry**       | One sale's commission, **captured when the sale happens**. Computing it at payout time would let a rate change silently rewrite history. A cancellation writes a second, sign-flipped line rather than deleting this one, and the mirror copies the recorded amounts for the same reason.                                                                                                                                                                                                                | ours          |
